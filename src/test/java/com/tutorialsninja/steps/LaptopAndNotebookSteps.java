...
public class LaptopAndNotebookSteps {
    ...
    public void iShouldSeeProductArrangeInPriceHighToLowOrderSuccessfully() {
        Assert.assertEquals("Product not sorted by price High to Low", new LaptopAndNotebookPage().priceHighToLow(), new LaptopAndNotebookPage().getPriceHighToLow().toString());
    }
    ...
    public void iFilledAllMandatoryFieldOfCheckoutPage() {
        new CheckoutPage().enterFirstName("Sneha");
        new CheckoutPage().enterLastName("Shah");
        new CheckoutPage().enterEmailId("sneha123@gmail.com");
        new CheckoutPage().enterTelephone("07546911532");
        new CheckoutPage().enterAddressLine1("9 Harrow Road");
        new CheckoutPage().enterCity("London");
        new CheckoutPage().enterPostCode("AB1 1AS");
