...
public class DesktopPageSteps {
    ...
    public void iShouldSeeProductArrangeInAlphabeticalOrderSuccessfully() {
        org.junit.Assert.assertEquals(String.valueOf(new DesktopPage().getSortedProductList()), new String[]{"Products not sorted as expected."}, new DesktopPage().expectedList());
    }
    ...
    public void iShouldNavigateToProductPage(String productName) {
        org.junit.Assert.assertEquals("HP LP3065 Product not display", productName, new ProductPage().getProductHPLP3065());
    ...
    public void iShouldSeeProductAddedSuccessfullyMessage() throws InterruptedException {
        Thread.sleep(500);
        org.junit.Assert.assertTrue("Product not added to cart", new ProductPage().getSuccessText().contains("Success: You have added HP LP3065 to your shopping cart!"));
    }
    ...
    public void iShouldSeeShoppingCartText() {
        org.junit.Assert.assertTrue("Shopping cart page not opened", new ShoppingCartPage().getShoppingCartText().contains("Shopping Cart"));
    }

    @And("^I should see \"([^\"]*)\" product name$")
    public void iShouldSeeProductName(String productName) {
        org.junit.Assert.assertEquals("Product name not matched", "HP LP3065", new ShoppingCartPage().getProductText());

    }

    @And("^I should see \"([^\"]*)\" delivery date$")
    public void iShouldSeeDeliveryDate(String deliveryDate) {
        org.junit.Assert.assertTrue("Delivery date not matched", new ShoppingCartPage().getDeliveryDateOfProduct().contains("2022-11-30"));

    }

    @And("^I should see \"([^\"]*)\" model name$")
    public void iShouldSeeModelName(String modelName) {
        org.junit.Assert.assertEquals("Model not matched", "Product 21", new ShoppingCartPage().getProductModel());
    }
